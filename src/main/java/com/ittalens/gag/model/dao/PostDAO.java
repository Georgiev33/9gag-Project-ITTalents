...
public class PostDAO {
    ...
    public List<PostRespDTO> getAllRecentPostsSortedByReactionCount(int offset, int pageSize){
        String sql = "SELECT p.id, p.title, p.resource_path, p.created_at, p.created_by, p.category_id, COUNT(r.post_id) AS number_of_reactions\n" +
                "FROM posts p LEFT JOIN users_posts_reactions r ON (p.id = r.post_id)\n" +
                "GROUP BY p.id HAVING p.created_at > DATE_ADD(CURDATE(), INTERVAL -7 DAY) ORDER BY number_of_reactions DESC LIMIT ?,?";
    ...
    public List<PostRespDTO> getAllRecentPostsByCategorySortedByReactionCount(int offset, int pageSize, long categoryId){
        String sql = "SELECT p.id, p.title, p.resource_path, p.created_at, p.created_by, p.category_id, COUNT(r.post_id) AS number_of_reactions\n" +
                "FROM posts p LEFT JOIN users_posts_reactions r ON (p.id = r.post_id)\n" +
                "GROUP BY p.id HAVING p.created_at > DATE_ADD(CURDATE(), INTERVAL -7 DAY) AND p.category_id = ? ORDER BY number_of_reactions DESC LIMIT ?,?";
        List<PostRespDTO> respDTOS = jdbcTemplate.query(sql, new PreparedStatementSetter() {
            @Override
            public void setValues(PreparedStatement ps) throws SQLException {
    ...
    public List<PostRespDTO> getAllRecentPostsByCategoryId(int offset, int pageSize, long categoryId){
        String sql = "SELECT p.id, p.title, p.resource_path, p.created_at, p.created_by, p.category_id\n" +
                "FROM posts p\n" +
    ...
    public List<PostRespDTO> getAllRecentPostsByTagIdSortedByReactionCount(int offset, int pageSize, long tagId){
        String sql = "SELECT p.id, p.title, p.resource_path, p.created_at, p.created_by, p.category_id, " +
                ...
                "GROUP BY p.id HAVING p.created_at > DATE_ADD(CURDATE(), INTERVAL -7 DAY) AND pwt.tag_id = ? ORDER BY number_of_reactions DESC LIMIT ?,?";
        List<PostRespDTO> respDTOS = jdbcTemplate.query(sql, new PreparedStatementSetter() {
            @Override
            public void setValues(PreparedStatement ps) throws SQLException {
                ps.setLong(1,tagId);
    ...
    public List<PostRespDTO> getAlLRecentPostsByTagId(int offset, int pageSize, long tagId){
        String sql = "SELECT p.id, p.title, p.resource_path, p.created_at, p.created_by, p.category_id, pwt.tag_id AS tag\n" +
                "FROM posts p\n" +
                "JOIN posts_with_tags pwt ON (p.id = pwt.post_id)\n" +
                "WHERE p.created_at > DATE_ADD(CURDATE(), INTERVAL -7 DAY) AND pwt.tag_id = ? ORDER BY created_at DESC LIMIT ?,?";
        List<PostRespDTO> respDTOS = jdbcTemplate.query(sql, new PreparedStatementSetter() {
                    @Override
                    public void setValues(PreparedStatement ps) throws SQLException {
                        ps.setLong(1,tagId);
